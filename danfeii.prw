...
#DEFINE MAXITEMC   070                                                // Máxima de caracteres por linha de produtos/serviços
...
User Function PrtNfeSef(cIdEnt,cVal1,cVal2,oDanfe,oSetup,cFilePrint,cVal3,_cDocPDF)
	...
	PRIVATE MV_PAR06 := 2
	PRIVATE _lAutPdf := IIf(valType(_cDocPDF)=="U",.F.,.T.)
	Private _cFilePdf := ""

	If _lAutPdf                     
		oDanfe := FwMSPrinter():New(('DANFE'+(_cAlias)->F2_DOC),6,.F.,,.T.)
		_cFilePdf := "\XMLAUT\"+('DANFE'+(_cAlias)->F2_DOC)+".pdf"
		oDanfe:lServer := .T.
		oDanfe:lInJob := .T.
	Else                                          
	...
	*/
	...
	If ( oDanfe:lInJob )
		...
		oDanfe:cPathPDF := "\XMLAUT\"

		DanfeProc(@oDanfe,.F.,cIdEnt,cVal1,cVal2,@lExistNfe)

	Else
		// ------------------------------------------------------------------------------------------
		// Execução via thread de smartcl?ient
		// ------------------------------------------------------------------------------------------
	...
	If !_lAutPdf
		...
		If lExistNfe
			...
			IF ( (SM0->M0_CODIGO == "14" .OR. SM0->M0_CODIGO == "25")  .And. alltrim(SM0->M0_CODFIL) == "01")  .OR. (SM0->M0_CODIGO == "24" .And. alltrim(SM0->M0_CODFIL) == "01")
				If MsgYesNo(" Deseja Imprimir Boleto ? ")
					U_RFINB01()
				Endif	   
			Endif   
		Else
...
Static Function DanfeProc(oDanfe,lEnd,cIdEnt,cVal1,cVal2,lExistNfe)
	...
	If !_lAutPdf                     
		_lContinua := Pergunte("NFSIGW",.T.)
		//    If Aviso("Escolha","Emitir em Qual Unidade ? ",{"PRIMEIRA","SEGUNDA"}) == 2
		//		_nUnidade := 2
		//	Endif	
	...
	Else                                    
		Pergunte("NFSIGW",.F.)
		MV_PAR01 := (_cAlias)->F2_DOC
		MV_PAR02 := (_cAlias)->F2_DOC
		MV_PAR03 := (_cAlias)->F2_SERIE
		MV_PAR04 :=  2       
		MV_PAR05 :=  2       
		MV_PAR06 :=  2       
					...
					If !Empty(aXML[nX][2]) .And. Empty((cAliasSFT)->FT_DTCANC)
						...
						If ( !Empty(cAutoriza) .Or. !Empty(cCodAutDPEC) .Or. Alltrim(aXML[nX][8]) $ "2,5,7" ) .And. !cCodRetNFE $ RetCodDene()
							...
							ElseIf aNotas[nX][02]=="S" .And. MV_PAR04==2 .And. (oNfe:_NFE:_INFNFE:_IDE:_TPNF:TEXT=="1")
								dbSelectArea("SF2")
								dbSetOrder(1)
								If MsSeek(xFilial("SF2")+PADR(aNotas[nX][05],TAMSX3("F2_DOC")[1])+aNotas[nX][04]) .And. Alltrim(aXML[nX][8])$"1,3,4,6" .Or. ( Alltrim(aXML[nX][8]) $ "2,5"  .And. !Empty(cAutoriza) )
									RecLock("SF2")
									If !SF2->F2_FIMP$"D"
										SF2->F2_FIMP := "S"
									EndIf
...
Static Function PrtDanfe(oDanfe,oNFE,cCodAutSef,cModalidade,oNfeDPEC,cCodAutDPEC,cDtHrRecCab,dDtReceb,aNota)
	...
	Local cChaveCont 	:= ""
	Local cLogo      	:= "\SYSTEM\msmdilogo.gif" //FisxLogo("1")
	Local cGuarda       := ""  
	Local cEsp		    := "" 
	Local cLogoD	    := ""
	local cEndDest      := ""
	...
	aadd(aSitSN,"900")
	      	...
	      	If FWCodEmp() == "24"
	      		cLogo := "\SYSTEM\MSMDILG24.BMP" //FisxLogo("1")
	      	Else 
	      		cLogo := "\SYSTEM\msmdilogo.gif"
	      	EndIf
	//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
	//³Quadro Destinatario                                                     ³
	//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
	...
	For nZ := 1 To nLenDet
				...
				For nY := 1 To nLenSit
					...
					If Type("oImposto:_Imposto:_ICMS:_ICMS"+aSitTrib[nPrivate2])<>"U"
						...
						ElseIf Type("oImposto:_Imposto:_ICMS:_ICMS"+aSitTrib[nPrivate2]+":_MOTDESICMS") <> "U" .And. Type("oImposto:_PROD:_VDESC:TEXT") <> "U"   //SINIEF 25/12, efeitos a partir de 20.12.12 
							If Type("oImposto:_PROD:_vICMS:TEXT") <> "U"
								nValICM  := Val(&("oImposto:_Imposto:_ICMS:_ICMS"+aSitTrib[nY]+":_vICMS:TEXT"))
							EndIf
						EndIf
		...
		AADD(aAux, AllTrim(TransForm(nVUnit,TM(nVUnit,TamSX3("D2_PRCVEN")[1],4))))
...
//			lPontilhado := .T. //IIf( nLenDet > 1, .T., lPontilhado )
		...
		Endif
		//FIM BLOCO TRATAMENTO UNIDADE TRIBUTARIA COMERCIAL FORTFRUIT
	...
	*/
	...
	oDanfe:Box(000,000,020,501)
	oDanfe:Say(010, 002, "RECEBEMOS DE "+NoChar(oEmitente:_xNome:Text,lConverte)+" OS PRODUTOS CONSTANTES DA NOTA FISCAL INDICADA ", oFont10N:oFont)
	oDanfe:Box(019,000,060,111)
	oDanfe:Say(029, 002, "DATA DE RECEBIMENTO", oFont10N:oFont)
	oDanfe:Box(019,110,060,500)
	oDanfe:Say(029, 112, "IDENTIFICAÇÃO E ASSINATURA DO RECEBEDOR", oFont10N:oFont)
	oDanfe:Box(000,500,060,603)
	oDanfe:Say(015, 512, "NF-e", oFont18N:oFont)
	oDanfe:Say(030, 510, "N. "+StrZero(Val(oIdent:_NNf:Text),9), oFont18N:oFont)
	oDanfe:Say(045, 510, "SÉRIE "+oIdent:_Serie:Text, oFont18N:oFont)


	//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
	//³Quadro 1 IDENTIFICACAO DO EMITENTE                                      ³
	//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
	oDanfe:Box(070,000,137,250)
	oDanfe:Say(080,098, "Identificação do emitente",oFont12N:oFont)
	nLinCalc	:=	095
	...
	If Type("oEmitente:_EnderEmit:_xCpl") <> "U"
		...
		oDanfe:Say(nLinCalc,098, oEmitente:_EnderEmit:_xMun:Text+"/"+oEmitente:_EnderEmit:_UF:Text + "Fone: "+IIf(Type("oEmitente:_EnderEmit:_Fone")=="U","",oEmitente:_EnderEmit:_Fone:Text),oFont08N:oFont)
		nLinCalc+=10
	...
	oDanfe:Box(070,248,137,351)
	oDanfe:Say(081,255, "D A N F E",oFont18N:oFont)
	//oDanfe:Say(065,258, "DOCUMENTO AUXILIAR DA",oFont07:oFont)
	//oDanfe:Say(075,258, "NOTA FISCAL ELETRÔNICA",oFont07:oFont)
	oDanfe:Say(090,266, "0-ENTRADA",oFont08:oFont)
	oDanfe:Say(100,266, "1-SAÍDA"  ,oFont08:oFont)

	oDanfe:Box(090,305,100,315)
	oDanfe:Say(097,307, oIdent:_TpNf:Text,oFont08N:oFont)
	...
	If nfolha==1
		If lMv_Logod
			oDanfe:SayBitmap(071,000,cLogoD,060,061)
		Else
			oDanfe:SayBitmap(071,000,cLogo,060,061)
		EndIF
	Endif           
	...
	oDanfe:Box(070,350,100,603)
	oDanfe:Box(100,350,120,603)
	...
	If _lContigencia
		oDanfe:Box(120,350,162,603)
	Else                          
		oDanfe:Box(120,350,137,603)
	Endif	

	//oDanfe:Box(105,350,137,603)

	If nFolha == 1
		oDanfe:Say(108,355,"CHAVE DE ACESSO DA NF-E",oFont12N:oFont)
		nFontSize := 28
		oDanfe:Say(116,355,TransForm(SubStr(oNF:_InfNfe:_ID:Text,4),"@r 9999 9999 9999 9999 9999 9999 9999 9999 9999 9999 9999"),oFont10N:oFont)
		oDanfe:Code128C(099,370,SubStr(oNF:_InfNfe:_ID:Text,4), nFontSize )
	EndIf
	...
	If Empty(cCodAutDPEC)
		If Empty(cChaveCont)
			oDanfe:Say(127,355,"Consulta de autenticidade no portal nacional da NF-e",oFont10N:oFont)
			oDanfe:Say(135,355,"www.nfe.fazenda.gov.br/portal ou no site da SEFAZ Autorizada",oFont10N:oFont)
		Endif
	Endif

	If  !Empty(cCodAutDPEC)
		oDanfe:Say(128,355,"Consulta de autenticidade no portal nacional da NF-e",oFont10N:oFont)
		oDanfe:Say(133,355,"www.nfe.fazenda.gov.br/portal ou no site da SEFAZ Autorizada",oFont10N:oFont)
	Endif
	...
	If !Empty(cChaveCont) .And. Empty(cCodAutDPEC) .And. !(Val(oNF:_INFNFE:_IDE:_SERIE:TEXT) >= 900)
		If nFolha == 1
			If !Empty(cChaveCont)
				nFontSize := 28
				oDanfe:Code128C(155,370,cChaveCont, nFontSize )
			EndIf
		Else
			If !Empty(cChaveCont)
				nFontSize := 28
	...
	Else
		...
		If !Empty(cCodAutDPEC)
			oDanfe:Say(148,300,"NÚMERO DE REGISTRO DPEC",oFont08N:oFont)
		Endif
		...
		If Empty(cCodAutDPEC) .And. (((Val(oNF:_INFNFE:_IDE:_SERIE:TEXT) >= 900).And.(oNFe:_NFE:_INFNFE:_IDE:_TPEMIS:TEXT)$"23") .Or. (oNFe:_NFE:_INFNFE:_IDE:_TPEMIS:TEXT)$"1")
			oDanfe:Say(148,302,"PROTOCOLO DE AUTORIZAÇÃO DE USO",oFont08N:oFont)
		Endif
