...
// Modal Component (Reutilizado do padrão FF)
...
export const FiscalHome = () => {
    ...
    const saveDate = () => {
        ...
        const newDate = new Date(parseInt(y), parseInt(m) - 1, parseInt(d), 12, 0, 0);
        setDate(newDate);
            ...
            <header className="sticky top-0 z-50 px-6 py-4">
                <div className="max-w-7xl mx-auto">
                    <div className="bg-white/80 dark:bg-slate-800/80 backdrop-blur-md rounded-2xl shadow-sm border border-white/20 dark:border-slate-700/50 px-6 py-3 flex items-center justify-between">
                        <div className="flex items-center gap-3 cursor-pointer" onClick={() => telaAtual === "MENU" ? navigate("/") : setTelaAtual("MENU")}>
                            <div className="bg-gradient-to-tr from-indigo-600 to-indigo-400 h-10 w-10 rounded-xl flex items-center justify-center text-white shadow-lg shadow-indigo-600/20">
                                <span className="font-bold text-xl italic tracking-tighter">SF</span>
                            </div>
                            <div>
                                <h1 className="text-lg font-bold leading-tight text-slate-800 dark:text-white">FiscalFF</h1>
                                <span className="text-[10px] font-semibold text-slate-400 uppercase tracking-widest">Portal Contábil</span>
                            </div>
                        </div>

                        <div className="flex items-center gap-2 md:gap-4">
                            {/* Seletor de Filial (Visível mesmo no menu) */}
                            <div className="hidden lg:flex flex-col items-end mr-4">
                                <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Filial Ativa</span>
                                <select
                                ...
                                >
                                  {FILIAIS.map((f) => (
                                    <option key={f.codigo} value={f.codigo} className="dark:bg-slate-800">
                                      {f.nome}
                                    </option>
                                  ))}
                                </select>
