  ...
  const handleExportar = async () => {
    ...
    try {
      ...
      if (response.ok && data.dados) {
        const dadosExcel = data.dados.map((r: B1Product) => ({
          "Código": r.B1_COD,
          "Descrição": r.B1_DESC,
          "Tipo": r.B1_TIPO,
          "POS/IPI (NCM)": r.B1_POSIPI,
          "TE (Entrada)": r.B1_TE,
          "TS (Saída)": r.B1_TS,
          "Bloqueio": r.B1_MSBLQL === "1" ? "Bloqueado" : "Desbloqueado",
          "Conta Contábil": r.B1_CONTA,
          "Origem": r.B1_ORIGEM,
          "Classif. Fiscal": r.B1_CLASFIS,
        }));

        const ws = XLSX.utils.json_to_sheet(dadosExcel);
        const wb = XLSX.utils.book_new();
        XLSX.utils.book_append_sheet(wb, ws, "Produtos SB1");
        XLSX.writeFile(wb, `Produtos_SB1140_${data.totalRegistros}_registros.xlsx`);
      } else {
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
          ...
          <div className="p-4 border-b bg-slate-50 flex justify-between items-center">
            ...
            <button
              ...
              disabled={exportando || carregando || totalRegistros === 0}
            ...
            >
              ...
              {exportando ? "Exportando..." : "Exportar Completo"}
            </button>
          </div>
