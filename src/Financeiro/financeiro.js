...
const ContasReceber = () => {
  ...
  return (
    <div className="min-h-screen bg-[#F3F4F6] dark:bg-[#0B1120] text-slate-800 dark:text-slate-100 font-sans pb-20 transition-colors duration-300">
      ...
      <AppHeader title="Financeiro" subtitle="Controle" icon="payments" iconGradient="from-emerald-500 to-green-400" iconShadow="shadow-emerald-500/20" onBack="/" />
        ...
        <div className="space-y-4">
          <div>
            ...
            >
              ...
              {vendedores.map((v, i) => (
                <option key={i} value={v.codigo}>{v.nome}</option>
              ))}
            </select>
          </div>
          <div>
